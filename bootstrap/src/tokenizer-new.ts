...
const isWhiteSpace = (char: string) => ' \r\t'.includes(char);
const isNewline = (char: string) => char === '\n';
const isNumber = (char: string) => NUMBER_CHARS.includes(char);
...
const tokenizer = (filename: string): TOKEN[] => {
  ...
  while (currenLoc.index < text.length) {
    ...
    const next2Chars = currentChar + text[currenLoc.index + 1];
    ...
    if (isWhiteSpace(currentChar)) {
      ...
      while (isNumber(text[currenLoc.index + 1])) advance();
